	<div class="contact-onscroll-open"> 
		<div class="contact-onscroll-open-tekst1">
			<p>Contact</p>
		</div>
		<div class="contact-onscroll-open-tekst2">
			<p>Meer weten?</p><p id="closecontact">X</p><br><p><i class="fas fa-phone"></i><i class="fab fa-whatsapp fa-lg"></i><a href="callto:0683916328" target="_blank">06 83 91 63 28</a><br>
			<p><i class="far fa-envelope"></i><a href="mailto:martina@balloonsss.nl" target="_blank">martina@balloonsss.nl</a></p>
			<a href="https://www.facebook.com/balloonsssveenendaal" target="_blank"><i class="fab fa-facebook-f fa-lg contactfbicon"></i></a>
			<a href="https://www.linkedin.com/company/balloonsssveenendaal/" target="_blank"><i class="fab fa-linkedin-in fa-lg"></i></a>
		</div>
	</div>